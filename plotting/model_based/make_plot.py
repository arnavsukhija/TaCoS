...
LABEL_NAMES = {
    'Optimistic': r'\textsc{OTaCoS}',
    'Pets': r'\textsc{PETS-TaCoS}',
    'Mean': r'\textsc{Mean-TaCoS}',
    'Standard RL': 'Standard RL',
}
...
data = pd.read_csv('data_pendulum/wtc_model_based_pendulum_num_measurement.csv')
...
data = pd.read_csv('data_pendulum/wtc_model_based_pendulum_episode_reward.csv')
...
for stats in episode_reward_stats:
    axs[0].plot(stats.xs, stats.ys_median, label=LABEL_NAMES[stats.name],
    ...
    axs[0].axhline(y=MODEL_FREE_REWARD, color='black', linestyle='-', label=r'\textsc{SAC-TaCoS}', linewidth=LINE_WIDTH)
    axs[0].set_xlabel('Episodes', fontsize=LABEL_FONT_SIZE)
    axs[0].set_ylabel('Episode Reward', fontsize=LABEL_FONT_SIZE)

    axs[0].set_title('Pendulum Swing-Up [Duration=10s]', fontsize=TITLE_FONT_SIZE, pad=40)
...
first_episode_for_policy_training = 0
NUM_SAMPLES = 5
MODEL_FREE_REWARD = 64.4880142211914
...
data = pd.read_csv('data_rccar/wtc_model_based_rccar_num_measurement.csv')
filtered_df = data[(data['max_time_factor'] == max_time_factor) &
                   (data['beta_factor'] == beta_factor) &
                   (data['transition_cost'] == transition_cost) &
                   (data['deterministic_policy_for_data_collection'] == deterministic_policy_for_data_collection) &
...
data = pd.read_csv('data_rccar/wtc_model_based_rccar_episode_reward.csv')
...
for stats in episode_reward_stats:
    axs[1].plot(stats.xs, stats.ys_median, label=LABEL_NAMES[stats.name],
                linewidth=LINE_WIDTH,
                linestyle=LINESTYLES_FROM_NAMES[stats.name], )
    ...
    axs[1].axhline(y=MODEL_FREE_REWARD, color='black', linestyle='-', label=r'\textsc{SAC-TaCoS}', linewidth=LINE_WIDTH)
    axs[1].set_xlabel('Episodes', fontsize=LABEL_FONT_SIZE)
    axs[1].set_title('RC Car [Duration=4s]', fontsize=TITLE_FONT_SIZE, pad=40)

    # axs[1].set_ylabel('Episode Reward', fontsize=LABEL_FONT_SIZE)
...
fig.tight_layout(rect=[0.0, 0.0, 1, 1.0])

plt.savefig('model_based_tacos.pdf')
plt.show()
